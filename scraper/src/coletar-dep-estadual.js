...
// Se não passou argumento, coleta AC (UF piloto)
const ufsParaProcessar = ufsAlvo.length ? ufsAlvo : ['AC'];

console.log(`=== INICIANDO COLETA DEPUTADO ESTADUAL / DISTRITAL 2026 ===`);
...
async function processarUf(uf, workerId) {
  ...
  try {
    const rotuloCargo = uf === 'DF' ? 'deputados distritais' : 'deputados estaduais';
    console.log(`[Worker ${workerId}] Buscando lista de ${rotuloCargo} em ${uf}...`);
    ...
    for (const [i, base] of lista.entries()) {
      const item = { ...base, foto_arquivo: `dep-estadual-${uf.toLowerCase()}-${base.slug}` };
      try {
        const perfil = await coletarPerfil(page, item);
        perfil.cargo = 'dep-estadual';
        perfil.uf = uf;
...
console.log(`\n=== RESUMO COLETA DEPUTADO ESTADUAL / DISTRITAL ===`);
console.log(`Total coletado nas UFs processadas: ${todos.length}`);
if (resumoErros.length > 0) {
  console.log(`Total de falhas/erros pontuais: ${resumoErros.length}`);
  await writeFile(path.join(OUT_DIR, 'dep-estadual-erros.json'), JSON.stringify(resumoErros, null, 2));
}
