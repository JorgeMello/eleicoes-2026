...
function executeSql(sql) {
  const tmpFile = path.join(__dirname, `tmp_update_est_${Date.now()}_${Math.random().toString(36).slice(2)}.sql`);
...
async function main() {
  console.log('=== ENRIQUECIMENTO DE PATRIMÔNIO & BENS - DEPUTADO ESTADUAL / DISTRITAL ===');

  // Consulta todos os candidatos a deputado estadual pendentes
  const query = `SELECT id, nome, uf, perfil_g1_url FROM eleicoes.candidatos WHERE cargo='dep-estadual' AND patrimonio_total IS NULL ORDER BY id ASC;`;
  ...
  if (candidatos.length === 0) {
    console.log('Todos os candidatos a Deputado Estadual já estão enriquecidos!');
    return;
  }
